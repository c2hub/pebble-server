...
impl fmt::Display for Packet {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "{}", Yellow.bold().paint("packet "));

		match *self {
			Packet::New => write!(f, "new"),
			Packet::Find { ref name, .. } => write!(f, "find {}", Green.bold().paint(name.clone())),
			Packet::Error { ref msg } => write!(f, "error {}", Red.bold().paint(msg.clone())),
			Packet::Upload {
				ref name,
				ref version,
				..
			...
			Packet::Update { ref data } => write!(f, "update {}", Red.bold().paint(data.clone())),
			Packet::Register { ref name, ref hash } => write!(
				f,
				"register {} {}",
				Green.bold().paint(name.clone()),
				Red.bold().paint(hash.clone())
			),
			Packet::Login { ref name, ref hash } => write!(
				f,
				"login {} {}",
				Green.bold().paint(name.clone()),
				Red.bold().paint(hash.clone())
